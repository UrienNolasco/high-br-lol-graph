...
describe('ChampionStatsService', () => {
  ...
  let tierRank: jest.Mocked<Pick<TierRankService, 'getPreviousPatch' | 'calculateChampionScore'>>;
  ...
  beforeEach(async () => {
    ...
    dataDragon = { getChampionById: jest.fn(), getChampionImageUrls: jest.fn() } as any;
    tierRank = { getPreviousPatch: jest.fn(), calculateChampionScore: jest.fn() } as any;
  ...
  it('should return paginated champion stats', async () => {
    ...
    dataDragon.getChampionById.mockReturnValue({ id: 'Annie', name: 'Annie', key: '1' });
    dataDragon.getChampionImageUrls.mockResolvedValue({ square: 'img.png', loading: 'l.png', splash: 'sp.png' });
    tierRank.calculateChampionScore.mockReturnValue({ score: 75, tier: 'S', hasInsufficientData: false });
  ...
  it('should handle champions with insufficient data', async () => {
    ...
    dataDragon.getChampionById.mockReturnValue({ id: 'Annie', name: 'Annie', key: '1' });
    dataDragon.getChampionImageUrls.mockResolvedValue({ square: 'img.png', loading: 'l.png', splash: 'sp.png' });
    tierRank.calculateChampionScore.mockReturnValue({ score: 0, tier: 'Dados Insuficientes', hasInsufficientData: true });
  ...
  it('should paginate correctly', async () => {
    const stats = Array.from({ length: 30 }, (_, i) => ({ ...mockStat, championId: i + 1 }));
    repo.findManyByPatch.mockResolvedValue(stats as any);
    tierRank.getPreviousPatch.mockReturnValue(null);
    dataDragon.getChampionById.mockReturnValue({ id: 'Champ', name: 'Champ', key: '1' });
    dataDragon.getChampionImageUrls.mockResolvedValue({ square: 'img.png', loading: 'l.png', splash: 'sp.png' });
    tierRank.calculateChampionScore.mockReturnValue({ score: 50, tier: 'A', hasInsufficientData: false });

    const result = await service.getChampionStats('15.1', 2, 10, 'winRate', 'desc');
